    ...
    return (
        <div className="max-w-full mx-auto space-y-4">
            <header className='flex justify-between items-center'>
                ...
                <button type='button' className='bg-music-primary py-2 px-7 rounded-lg' onClick={() => setOpen(true)}>+ Add Song</button>
            </header>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {songs.map((song) => (
                    <div
                        key={song._id}
                        className="bg-card rounded-xl border border-border/40 p-6 text-center cursor-pointer hover:shadow-lg flex justify-center gap-4 items-center transition-shadow"
                    ...
                    >
                        ...
                        <button disabled={currentPlaying?._id === song._id} className="disabled:opacity-50 disabled:cursor-not-allowed px-4 py-2 bg-music-primary text-white rounded-lg text-sm font-medium hover:bg-music-primary/90 transition-colors">
                            Play Now
                        </button>
                    </div>
                ))}
