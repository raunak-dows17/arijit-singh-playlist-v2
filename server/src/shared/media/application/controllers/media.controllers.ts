    ...
    try {
      if (file) {
        ...
        req.uploadedMedia =
          response.data?.type === "single"
            ? (response.data?.item as MediaEntity)
            : (response.data?.items[0] as MediaEntity);
      }
      ...
      if (files && Array.isArray(files) && files.length > 0) {
        const response = await Promise.all(
          files.map(
            async (f, index) =>
              await this.mediaOperationUsecase.execute({
                entity: "Media",
                type: "create",
                data: {
        ...
        req.uploadedMedias = response.map<MediaEntity>((r) =>
          r.data?.type === "single"
